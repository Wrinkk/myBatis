...
//    Logger logger = LoggerFactory.getLogger(getClass());
    ...
    @GetMapping("/{menuCode}") // 변수상에 있는걸 바로 받아내는기술 passVariable
...
//    }
    ...
    /* 설명. 페이징 처리 후 */
    @GetMapping("/list")        // data.domain import
    ...
    /* 설명.
     *  @ResponseBody가 붙였을 때 기존과 다른 핸들러 메소드의 차이점
     *  1. 핸들러 메소드의 반환형이 어떤 것이라도 상관없다 (-> 모두 json 문자열 형태로 요청이 들어온 곳으로 반환된다.)
     *  2. 한글이 포함된 데이터는 prodeuces 속성에 application/json' 라는 MIME 타입과 'charset=UTF-8' 인코딩 타입을 붙여준다.
     * */
    @ResponseBody   // RestController를 쓰면 그안에 포함하고있음.
