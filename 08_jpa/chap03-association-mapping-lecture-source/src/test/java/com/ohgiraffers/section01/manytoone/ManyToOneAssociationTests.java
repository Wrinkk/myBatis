...
public class ManyToOneAssociationTests {
     ...
     * 설명.
     *  1. 다중성에 의한 분류
     *      : 연관 관계가 있는 개체 관계에서 실제로 연관을 가지는 객체의 수에 따라 분류
     *        - N:1(ManyToOne) 연관관계
     *        - 1:N(OneToMany) 연관관계
     *        - 1:1(OneToOne) 연관관계
     *        - N:N(ManyToMany) 연관관계
     ...
     *         개체 간의 연관 관계를 양방향으로 만들고 싶을 경우 반대 쪽에서도 필드를 추가해서 참조를
    ...
    void 다대일_연관관계_객체_그래프_탐색을_이용한_조회_테스트() {
        ...
        ManyAndCategory foundMenu = entityManager.find(ManyAndCategory.class, menuCode);
