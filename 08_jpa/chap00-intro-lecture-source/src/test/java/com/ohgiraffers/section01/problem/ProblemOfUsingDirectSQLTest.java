...
public class ProblemOfUsingDirectSQLTest {
    ...
    @BeforeEach
        // jdbc템플릿의 getconnectionㅣ 메소드 역할
    ...
    /* 설명. JDBC API를 이용해 직접 SQL을 다룰 떄 발생할 수 있는 문제점
    ...
    @DisplayName("직접 SQL을 작성하여 메뉴를 조회 할 때 발생하는 문제 확인")
    @Test
    ...
    void testChangeSelectColumns() throws SQLException {
        ...
        while (rset.next()) {
            ...
            menu.setMenuCode(rset.getInt("MENU_CODE"));
            menu.setMenuName(rset.getString("MENU_NAME"));
            menu.setMenuPrice(rset.getInt("MENU_PRICE"));       // 얘를 지운다면 위의 쿼리에서도 지워야함.
     ...
     *  객체지향에서 말하는 가지고 있는(ASSOCIATION 연관관계 혹은 COLLECTION 연관관계) 경우 데이터 베이스 저장 구조와
     *  다른 형태이다.
     * 설명.
     *  public class Menu {
    ...
    /* 설명. 4. 동일성 보장 문제 */
    @DisplayName("조회한 두 개의 행을 담은 객체의 동일성 비교테스트")
    @Test
